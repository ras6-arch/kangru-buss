...
def graphql(query,variables):
    ...
    req=urllib.request.Request(PEATUS_API,data=body,headers={'Content-Type':'application/json','User-Agent':'kangru-buss/1.0'})
    with urllib.request.urlopen(req,timeout=30) as r:data=json.load(r)
    if data.get('errors'):raise RuntimeError('Peatus API: '+json.dumps(data['errors'],ensure_ascii=False))
    return data['data']
def sec_dt(service_day,seconds):
    return datetime.fromtimestamp(service_day,TZ)+timedelta(seconds=seconds)
...
def peatus_today_entries():
    ...
    for home in sorted(HOME_STOPS):
        candidates=graphql(stop_query,{'name':home})['stops']
        exact=next((s for s in candidates if normalize_stop(s['name'])==home),None)
        if not exact:continue
        deps=graphql(dep_query,{'id':exact['gtfsId'],'n':50,'startTime':start})['stop']['stoptimesWithoutPatterns']
        for dep in deps:
            line=(dep.get('trip') or {}).get('route',{}).get('shortName')
            trip_id=(dep.get('trip') or {}).get('gtfsId')
            if line not in LINES or not trip_id or trip_id in used:continue
            used.add(trip_id)
            trip=graphql(trip_query,{'id':trip_id})['trip']
            ...
            for hi in home_idxs:
                ...
                for cs in sts[hi+1:]:
                    ...
                    arrdt=sec_dt(cs['serviceDay'],cs['scheduledArrival'])
                    key=(depdt.isoformat(),line,'city',home,city)
...
raw=urllib.request.urlopen(GTFS_URL,timeout=60).read();z=zipfile.ZipFile(io.BytesIO(raw))
routes=read_csv(z,'routes.txt');trips=read_csv(z,'trips.txt');stops=read_csv(z,'stops.txt');stop_times=read_csv(z,'stop_times.txt');calendar=read_csv(z,'calendar.txt') if 'calendar.txt' in z.namelist() else [];calendar_dates=read_csv(z,'calendar_dates.txt') if 'calendar_dates.txt' in z.namelist() else []
...
now=datetime.now(TZ);limit=now+timedelta(hours=4);found=[e for e in entries if e['direction']=='city' and e['destination']=='Viru' and now<=datetime.fromisoformat(e['dep_iso'])<=limit]
print('CHECK Viru next4h:',[(e['line'],e['origin'],e['departure'],e['arrival']) for e in found])
